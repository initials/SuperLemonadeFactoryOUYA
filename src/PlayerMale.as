package
{
	...
	public class PlayerMale extends Character
	{
		...
		public function PlayerMale(X:int,Y:int,playerNumber:int, talkString:String)
		{
			...
			addAnimation("run", [6, 7, 8, 9, 10, 11 ], 24, true);
		...
		{			
			...
			if ( (FlxG.keys.pressed(Registry.p1Action)|| FlxG.joystick.j1ButtonXPressed || FlxG.ouyaController.u.held) && this._curAnim.name!="death" && this._currentlyControlled && !FlxG.keys.pressed(Registry.p1Down) && (levelOver==false) ) { // && ! this._curAnim.name=="death"
				this._talk = false;
				airDash += FlxG.elapsed;
				if (FlxG.keys.justPressed(Registry.p1Action) || FlxG.joystick.j1ButtonXJustPressed || FlxG.ouyaController.u.pressed ) {
